...
export class PolygonEditor extends Operator<Polygon> {
  ...
  private _handleMouseLeave = this._createControlledListener((e: MouseEvent) => {
    this.pen.draw(
      ...
      this._createShape(this.wipPoints?.concat({ x: e.offsetX, y: e.offsetY }) || []),
    );
  });
  ...
  private _handleMouseMove = this._createControlledListener((e: MouseEvent) => {
    if (this.pen) {
      if (this.state === State.Drawing) {
        ...
        //   points: this.wipPoints?.concat({ x: e.offsetX, y: e.offsetY })
        // });
        this.pen.draw(
          this.shapes,
          -1,
        ...
        */
        ...
        this.pen.drawTooltip(e.offsetX, e.offsetY, getTooltipText());
      }
        ...
        if (
          ...
          selectShape.data.points[this.resizeHandle].x = e.offsetX;
          selectShape.data.points[this.resizeHandle].y = e.offsetY;
          this.pen.draw(this.shapes, this.activeShapeIndex);
        ...
        if (this.shapes.length < this.options.editableMaxSize && hits.length === 0) {
          ...
          if (hits.length > 0) {
            ...
            if (this.shapes.length >= this.options.editableMaxSize) {
        ...
        if (selectShape) {
          ...
          if (result?.[0] === HitPointType.KeyPoint) {
            ...
            this.canvas.style.cursor = 'pointer';
            this.pen.drawTooltip(e.offsetX, e.offsetY, '单击在该位置添加节点');
          }
          else {
    ...
    if (this.pen) {
      if (this.state !== State.Resize && this.state !== State.Pan) {
        if (this.state === State.Drawing) {
          ...
          this._addPoint({ x: e.offsetX, y: e.offsetY });
        }
        else {
          const hits = this.measurer.isPointInPath({ x: e.offsetX, y: e.offsetY }, this.shapes);
          if (hits.length > 0) {
            ...
            if (
              ...
              && this.options.editableMaxSize > this.shapes?.length
            ) {
              this._addPoint({ x: e.offsetX, y: e.offsetY });
